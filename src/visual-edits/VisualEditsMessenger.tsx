...
const postMessageDedup = (data: any) => {
  ...
  if (window.parent === window) return; // Not in iframe, skip postMessage
...
export default function HoverReceiver() {
  ...
  // Initialize state without accessing localStorage (SSR safe)
  ...
  const onClickCleanupTimeoutRef = useRef<number | null>(null);
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    if (element !== editingElementRef.current) {
      logger.warn("Attempting to handle text change for non-editing element");
      return;
    }
  ...
  useEffect(() => {
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      if (hit) {
        ...
        if (onClickCleanupTimeoutRef.current) {
          clearTimeout(onClickCleanupTimeoutRef.current);
        }
        onClickCleanupTimeoutRef.current = window.setTimeout(() => {
    ...
    return () => {
      ...
      if (onClickCleanupTimeoutRef.current) clearTimeout(onClickCleanupTimeoutRef.current);
      // Clean up all persistent font timeouts
      persistentFontTimeouts.current.forEach((timeoutId) => {
        clearTimeout(timeoutId);
      });
      persistentFontTimeouts.current.clear();
    };
